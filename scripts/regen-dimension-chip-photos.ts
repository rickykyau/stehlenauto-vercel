...
const CHIPS: { slug: string; subject: string; framing: string; intent: string }[] = [
  ...
  //     and sees a true 1:1 swap.
  {
    ...
    subject:
      "2024 Ford F-150 SuperCrew 4-door pickup truck in BASE / XL WORK-TRUCK trim. Oxford white paint. Factory MATTE BLACK FRONT BUMPER (no chrome). Factory BLACK two-bar grille with BLACK surround (NO chrome anywhere on the grille). BLACK mirror caps and door handles. Factory halogen headlights with black bezels (no LED, no DRL). Factory 17-inch SILVER PAINTED STEEL WHEELS with simple plastic hubcaps. Standard ride height (no lift). NO aftermarket parts of any kind. NO chase rack, NO bed accessories, NO roof rack, NO light bar. This is the FLEET / COMMERCIAL utility look — function-over-form.",
  ...
  },
  {
    ...
    subject:
      "2024 Ford F-150 SuperCrew 4-door pickup truck in MID-TIER / XLT trim. Atlas blue metallic paint. Factory CHROME front bumper (NOT body-color). Factory BLACK two-bar grille with a CHROME SURROUND around it (this chrome surround is the SIGNATURE visual difference vs the BASE trim — look at the grille frame). CHROME mirror caps and chrome door handles. Factory halogen headlights with CHROME bezels. Factory 17-inch SILVER-PAINTED ALUMINUM ALLOY WHEELS (different style from BASE's steel wheels — these have proper alloy spokes). Standard ride height. NO aftermarket parts.",
  ...
  },
  {
    ...
    subject:
      "2024 Ford F-150 SuperCrew 4-door pickup truck in HIGH / PREMIUM trim — F-150 LIMITED or PLATINUM level (NOT Raptor, NOT Trail Boss, NOT Rebel, NOT Tremor — those are off-road packages, this is the LUXURY package). Agate black metallic paint. Body-color (black) front bumper. SATIN-CHROME single-bar grille with a unique fine-mesh insert and PLATINUM badge in the lower corner — this is the PREMIUM grille treatment, completely different from the BASE black two-bar and the MID black + chrome-surround. SATIN-CHROME mirror caps. Premium QUAD-BEAM LED HEADLIGHTS with chrome bezels (NOT halogen). Factory 22-INCH POLISHED ALUMINUM WHEELS — large, premium, polished bright finish (NOT off-road wheels, NOT matte black). Standard ride height — this is a LUXURY trim, not a lifted off-road truck. NO chase rack, NO bed accessories, NO roof rack.",
